...
export function Shell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-muted/35 md:grid md:grid-cols-[240px_1fr]">
    ...
    <main className="min-w-0 p-4 sm:p-6 lg:p-10"><header className="mb-8 flex items-center justify-end border-b pb-4"><div className="flex items-center gap-3"><UserCircle className="size-9 shrink-0 text-muted-foreground" /><div className="flex min-h-9 max-w-52 flex-col justify-center text-right leading-tight"><p className="truncate text-sm font-medium">{accountLabel(user?.email)}</p><p className="mt-1 text-xs text-muted-foreground">{user && roleLabels[user.role]}</p></div><Button variant="outline" size="sm" onClick={() => void logout()}><LogOut className="size-4" />Cerrar sesión</Button></div></header>{children}</main>
  </div>
}
