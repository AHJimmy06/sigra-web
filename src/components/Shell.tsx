...
export function Shell({ children }: { children: ReactNode }) {
  ...
  return <div className={`min-h-screen bg-muted/35 md:grid ${shellColumns}`}>
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 w-64 border-r bg-background shadow-xl transition-transform md:static md:z-auto md:w-auto md:shadow-none ${mobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
      ...
      <nav className={`space-y-1 px-3 py-5 ${collapsed ? 'md:px-2' : 'xl:px-3'}`}>
        {links.map(([to, label, Icon]) => <NavLink key={to} to={to} title={collapsed ? label : undefined} onClick={() => setMobileOpen(false)} className={({ isActive }) => `flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium ${collapsed ? 'md:justify-center md:px-2' : 'md:justify-center md:px-2 xl:justify-start xl:px-3'} ${isActive ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`}><Icon className="size-4 shrink-0" /><span className={collapsed ? 'md:hidden' : 'md:hidden xl:inline'}>{label}</span></NavLink>)}
      </nav>
      <Button type="button" variant="ghost" size="icon" className="absolute bottom-4 right-2 hidden md:inline-flex xl:hidden" aria-label="Expandir menú lateral" title="Expandir menú lateral" onClick={() => setCollapsed(false)}><ChevronRight className="size-4" /></Button>
    </aside>
    <main className="min-w-0 p-4 sm:p-6 lg:p-10"><header className="mb-8 flex items-center justify-between gap-3 border-b pb-4"><Button type="button" variant="outline" size="icon" className="md:hidden" aria-label="Abrir menú" title="Abrir menú" onClick={() => setMobileOpen(true)}><Menu className="size-4" /></Button><Button type="button" variant="ghost" size="icon" className="hidden md:inline-flex xl:hidden" aria-label="Colapsar menú lateral" title="Colapsar menú lateral" onClick={() => setCollapsed(true)}><ChevronLeft className="size-4" /></Button><div className="ml-auto flex items-center gap-3"><UserCircle className="size-9 shrink-0 text-muted-foreground" /><div className="flex min-h-9 max-w-52 flex-col justify-center text-right leading-tight"><p className="truncate text-sm font-medium">{accountLabel(user?.email)}</p><p className="mt-1 text-xs text-muted-foreground">{user && roleLabels[user.role]}</p></div><Button variant="outline" size="sm" onClick={() => void logout()}><LogOut className="size-4" /><span className="hidden sm:inline">Cerrar sesión</span></Button></div></header>{children}</main>
  </div>
}
