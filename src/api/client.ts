...
function defaultErrorMessage(code: ApiErrorCode): string {
  const messages: Record<ApiErrorCode, string> = {
    ...
    UNAUTHORIZED: "La sesión no es válida o ha vencido.",
    FORBIDDEN: "No tiene permisos para realizar esta acción.",
    NOT_FOUND: "No se encontró el recurso solicitado.",
    CONFLICT: "La solicitud entra en conflicto con los datos existentes.",
    RATE_LIMITED:
      "Se realizaron demasiadas solicitudes. Inténtelo de nuevo más tarde.",
    NETWORK_ERROR: "No fue posible conectar con el servidor.",
    TIMEOUT: "El servidor tardó demasiado en responder.",
    UNKNOWN_ERROR: "No fue posible completar la solicitud.",
  };
...
export async function api<T>(
  ...
  if (!response.ok) {
    ...
    const message = Array.isArray(payload.message)
      ...
      : (payload.message ?? defaultErrorMessage(code));
    if (response.status === 401)
      window.dispatchEvent(new CustomEvent("sigra:session-expired"));
