...
export function AnnouncementsPage() {
  ...
  const load = () =>
    api<PaginatedResponse<Announcement> | Announcement[]>("/announcements")
      .then((res) => {
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(
        e instanceof Error ? e.message : "No fue posible publicar el anuncio.",
      );
    } finally {
      setCreateBusy(false);
    }
  ...
  async function update(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(
        e instanceof Error ? e.message : "No fue posible editar el anuncio.",
      );
    } finally {
      setEditBusy(false);
    }
  ...
  async function setPublished(item: Announcement, published: boolean) {
    ...
    setError("");
    try {
      await api(`/announcements/${item.id}`, {
        method: "PATCH",
  ...
  return (
    <>
      <PageHeader
        ...
        description="Redacte y publique anuncios de texto para los residentes."
        action={
      ...
      >
        ...
        <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_180px]">
          <label className="relative">
            <span className="sr-only">Buscar en anuncios</span>
            ...
            <input
              ...
              placeholder="Ej. mantenimiento o ascensor"
      ...
      <Modal
        ...
        title="Crear anuncio"
        description="Complete los campos obligatorios. Marque la opción de publicación solo si desea mostrarlo inmediatamente."
      >
        <form
          onSubmit={create}
        ...
        >
          ...
          <label className="grid gap-1 text-sm font-medium">
            ...
            <textarea
              ...
              maxLength={2000}
              rows={5}
              placeholder="Ej. El ascensor estará fuera de servicio el sábado..."
              className="w-full rounded-lg border px-3 py-2 font-normal"
            />
          </label>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button type="submit" disabled={createBusy}>
              <Plus className="size-4" />
              {createBusy ? "Creando…" : "Crear anuncio"}
            </Button>
          </div>
        </form>
      </Modal>
      <Modal
        ...
        description="Actualice el título y el contenido del comunicado. El estado de publicación se gestiona mediante una confirmación separada."
      >
        <form
          onSubmit={update}
          onInvalid={setSpanishValidationMessage}
                ...
                >
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        setPendingPublication({ item, published: !isPublished })
                      }
                    >
                      {isPublished ? (
                        <>
      ...
      <ConfirmDialog
        ...
        description={
          pendingPublication
            ? `¿Desea ${pendingPublication.published ? "publicar" : "retirar"} el anuncio “${pendingPublication.item.title}”?`
            : ""
        }
        confirmLabel={
          pendingPublication?.published
        ...
        destructive={pendingPublication ? !pendingPublication.published : false}
      />
    </>
  );
}
