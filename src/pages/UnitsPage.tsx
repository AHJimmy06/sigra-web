...
export function UnitsPage() {
  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    ...
    setLoading(true); setError(''); setErrorKind('general')
    try {
    ...
    } catch (value) { if (!signal?.aborted && owner === listGeneration.current) { setItems([]); setTotal(0); setError(value instanceof Error ? value.message : 'No fue posible cargar las unidades.'); setErrorKind(value instanceof ApiError && value.status === 401 ? 'auth' : value instanceof ApiError && value.status === 403 ? 'forbidden' : 'general') } }
  ...
  function dismissAction() {
    ...
    actionController.current?.abort()
    setPendingToggle(null); setPendingLifecycle(null)
    setToggleBusy(false); setToggleError('')
  }
  function openAction(item: Unit, kind: 'toggle' | 'lifecycle') { dismissAction(); if (kind === 'toggle') setPendingToggle(item); else setPendingLifecycle({ item, action: item.archivedAt ? 'restore' : 'archive' }) }
