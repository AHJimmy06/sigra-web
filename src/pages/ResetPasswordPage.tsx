...
export function ResetPasswordPage() {
  ...
  return <main className="grid min-h-screen place-items-center bg-muted/40 p-4"><section className="w-full max-w-md rounded-2xl border bg-card p-7 shadow-sm">
    ...
    {submitted ? <div role="status" className="rounded-lg border border-emerald-500/30 bg-emerald-50 p-4 text-sm text-emerald-900">La contraseña se actualizó correctamente. Será redirigido al inicio de sesión.</div> : <form onSubmit={submit} onInvalid={validateFieldOnInvalid} onInput={validateFieldOnInput} onFocus={validateFieldOnFocus} className="grid gap-4"><label className="grid gap-1 text-sm font-medium">Nueva contraseña <span className="font-normal text-muted-foreground">(obligatorio, mínimo 8 caracteres)</span><input name="password" required type="password" minLength={8} maxLength={72} autoComplete="new-password" placeholder="Mínimo 8 caracteres" value={password} onChange={(event) => setPassword(event.target.value)} className="mt-1 w-full rounded-lg border bg-background px-3 py-2 font-normal" /></label><label className="grid gap-1 text-sm font-medium">Confirmar contraseña <span className="font-normal text-muted-foreground">(obligatorio)</span><input name="confirmation" required type="password" minLength={8} maxLength={72} autoComplete="new-password" placeholder="Repita la nueva contraseña" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} className="mt-1 w-full rounded-lg border bg-background px-3 py-2 font-normal" /></label><Button type="submit" disabled={busy}><Save className="size-4" />{busy ? 'Guardando…' : 'Guardar contraseña'}</Button></form>}
  </section></main>
}
