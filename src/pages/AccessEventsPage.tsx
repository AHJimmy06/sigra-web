...
export function AccessEventsPage() {
  ...
  const load = () =>
    api<PaginatedResponse<AccessEvent> | AccessEvent[]>("/access-events")
      .then((res) => {
  ...
  const filteredItems = useMemo(
    () =>
      items.filter((item) => {
        ...
        const matchesQuery =
          !normalizedQuery ||
          item.resident?.name.toLocaleLowerCase().includes(normalizedQuery) ||
          item.resident?.unitCode
            ?.toLocaleLowerCase()
            .includes(normalizedQuery) ||
          item.guard?.email.toLocaleLowerCase().includes(normalizedQuery);
