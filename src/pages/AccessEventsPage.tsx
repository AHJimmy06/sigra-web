...
export function AccessEventsPage() {
  ...
  const filteredItems = useMemo(() => items.filter((item) => {
    ...
    const eventDate = item.occurredAt.slice(0, 10)
    const matchesFrom = !fromDate || eventDate >= fromDate
    const matchesTo = !toDate || eventDate <= toDate
    return matchesQuery && matchesDecision && matchesDirection && matchesFrom && matchesTo
  }), [items, query, decisionFilter, directionFilter, fromDate, toDate])
  const pageSize = 10
  ...
  return <><PageHeader title="Historial de accesos" description="Consulte las decisiones registradas por la garita y supervise el flujo de entradas y salidas." />{error && <ErrorState message={error} />}
    <section className="mb-6 rounded-xl border bg-card p-4" aria-label="Filtros del historial de accesos"><div className="mb-3 flex items-center gap-2 text-sm font-medium"><Filter className="size-4" />Buscar y filtrar eventos</div><div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_180px_160px_150px_150px]"><label className="relative"><span className="sr-only">Buscar residente, unidad o guardia</span><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1) }} placeholder="Ej. Ana García o Torre A-101" className="w-full rounded-lg border bg-background py-2 pl-9 pr-3" /></label><label><span className="sr-only">Filtrar por decisión</span><select value={decisionFilter} onChange={(event) => { setDecisionFilter(event.target.value as keyof typeof decisionLabels); setPage(1) }} className="w-full cursor-pointer rounded-lg border bg-background px-3 py-2">{Object.entries(decisionLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label><span className="sr-only">Filtrar por dirección</span><select value={directionFilter} onChange={(event) => { setDirectionFilter(event.target.value); setPage(1) }} className="w-full cursor-pointer rounded-lg border bg-background px-3 py-2"><option value="ALL">Ambas direcciones</option><option value="ENTRY">Entradas</option><option value="EXIT">Salidas</option></select></label><label><span className="sr-only">Fecha inicial</span><input type="date" aria-label="Fecha inicial" value={fromDate} max={toDate || undefined} onChange={(event) => { setFromDate(event.target.value); setPage(1) }} className="w-full cursor-pointer rounded-lg border bg-background px-3 py-2" /></label><label><span className="sr-only">Fecha final</span><input type="date" aria-label="Fecha final" value={toDate} min={fromDate || undefined} onChange={(event) => { setToDate(event.target.value); setPage(1) }} className="w-full cursor-pointer rounded-lg border bg-background px-3 py-2" /></label></div></section>
