...
export function LoginPage() {
  ...
  return <main className="grid min-h-screen place-items-center bg-muted/40 p-4"><form id="login-form" onSubmit={submit} onInvalid={validateFieldOnInvalid} onInput={validateFieldOnInput} className="w-full max-w-sm rounded-2xl border bg-card p-7 shadow-sm">
    ...
    <label className="mb-6 block text-sm font-medium">Contraseña<input name="password" className="mt-2 w-full rounded-lg border bg-background px-3 py-2" type="password" minLength={8} autoComplete="current-password" placeholder="Escriba su contraseña" required value={password} onChange={(e) => setPassword(e.target.value)} /></label>
