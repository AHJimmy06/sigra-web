...
export function GuardScannerPage() {
  ...
  return <><PageHeader title="Escáner QR para guardias" description="La validación en línea comprueba la autenticidad temporal y registra cada decisión." />{error && <ErrorState message={error} />}
    <div className="grid gap-6 lg:grid-cols-[1fr_340px]"><section className="rounded-xl border bg-card p-4"><div id="qr-reader" className="min-h-72 overflow-hidden rounded-lg bg-muted" /><div className="mt-4 flex flex-wrap gap-3"><select aria-label="Dirección del acceso" className="rounded-lg border bg-background px-3 py-2" value={direction} onChange={(e) => setDirection(e.target.value as 'ENTRY' | 'EXIT')} disabled={running}><option value="ENTRY">Entrada</option><option value="EXIT">Salida</option></select>{running ? <Button variant="outline" onClick={() => void stop()}>Detener cámara</Button> : <Button onClick={() => void start()}>Iniciar cámara</Button>}</div></section>
