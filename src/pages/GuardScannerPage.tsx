...
export function GuardScannerPage() {
  ...
  const [running, setRunning] = useState(false); const [cameraBusy, setCameraBusy] = useState(false); const [online, setOnline] = useState(() => navigator.onLine); const [direction, setDirection] = useState<'ENTRY' | 'EXIT'>('ENTRY'); const [result, setResult] = useState<Result>(); const [error, setError] = useState('')
  useEffect(() => { const handleOnline = () => setOnline(true); const handleOffline = () => setOnline(false); window.addEventListener('online', handleOnline); window.addEventListener('offline', handleOffline); return () => { window.removeEventListener('online', handleOnline); window.removeEventListener('offline', handleOffline); if (scanner.current?.isScanning) void scanner.current.stop() } }, [])
  async function submit(payload: string) { if (processing.current) return; if (!navigator.onLine) { setError('No hay conexión con la API. El acceso no puede validarse sin conexión.'); return } processing.current = true; setError(''); try { const value = await api<Result>('/access/validate', { method: 'POST', body: JSON.stringify({ qrPayload: payload, clientEventId: crypto.randomUUID(), direction }) }); setResult(value) } catch (value) { setError(value instanceof ApiError ? value.message : 'No fue posible validar el acceso.') } finally { window.setTimeout(() => { processing.current = false }, 1500) } }
  ...
  return <><PageHeader title="Escáner QR para guardias" description="La validación en línea comprueba la autenticidad temporal y registra cada decisión." />{error && <ErrorState message={error} />}
    <div className="grid gap-6 lg:grid-cols-[1fr_340px]"><section className="rounded-xl border bg-card p-4"><div className={`mb-4 rounded-lg border p-3 text-sm ${online ? 'border-emerald-500/30 bg-emerald-50 text-emerald-900' : 'border-amber-500/40 bg-amber-50 text-amber-900'}`} role="status">{online ? 'Conexión disponible. Las validaciones se consultarán en la API.' : 'Sin conexión. Detenga el escáner: no es posible autorizar accesos offline.'}</div><div id="qr-reader" className="min-h-72 overflow-hidden rounded-lg bg-muted" /><div className="mt-4 flex flex-wrap items-center gap-3"><select aria-label="Dirección del acceso" className="cursor-pointer rounded-lg border bg-background px-3 py-2" value={direction} onChange={(e) => setDirection(e.target.value as 'ENTRY' | 'EXIT')} disabled={running || cameraBusy}><option value="ENTRY">Entrada</option><option value="EXIT">Salida</option></select>{running ? <Button variant="outline" disabled={cameraBusy} onClick={() => void stop()}><CameraOff className="size-4" />{cameraBusy ? 'Deteniendo…' : 'Detener cámara'}</Button> : <Button disabled={cameraBusy || !online} onClick={() => void start()}><Camera className="size-4" />{cameraBusy ? 'Iniciando…' : 'Iniciar cámara'}</Button>}<span className="text-sm text-muted-foreground">{running ? 'Cámara activa. Apunte al código QR.' : 'Cámara detenida.'}</span></div></section>
