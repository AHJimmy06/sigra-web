import { render, screen } from '@testing-library/react'
import { beforeEach, describe, expect, it, vi } from 'vitest'
import type { ReactNode } from 'react'
import App from './App'

const auth = vi.hoisted(() => {
  const state = { user: null as null | { role: 'ADMIN' | 'GUARD' | 'RESIDENT' }, loading: false, invalidateSession: vi.fn() }
  state.invalidateSession.mockImplementation(() => { state.user = null })
  return state
})
vi.mock('@/auth/AuthContext', () => ({ AuthProvider: ({ children }: { children: ReactNode }) => children, useAuth: () => auth }))
vi.mock('@/components/Shell', () => ({ Shell: ({ children }: { children: ReactNode }) => <>{children}</> }))
vi.mock('@/pages/DashboardPage', () => ({ DashboardPage: () => <div>Admin dashboard</div> }))
vi.mock('@/pages/GuardScannerPage', () => ({ GuardScannerPage: () => <div>Guard scanner</div> }))
vi.mock('@/pages/AccessEventsPage', () => ({ AccessEventsPage: () => <div>Access history</div> }))
vi.mock('@/pages/ResidentDetailPage', () => ({ ResidentDetailPage: () => <div>Resident detail</div> }))

describe('role routes', () => {
  beforeEach(() => {
    auth.user = null
    auth.invalidateSession.mockClear()
    window.history.pushState({}, '', '/')
  })

  it('routes ADMIN users to administration and blocks guard routes', async () => {
    auth.user = { role: 'ADMIN' }
    window.history.pushState({}, '', '/guard')
    render(<App />)
    expect(await screen.findByText('Admin dashboard')).toBeInTheDocument()
    expect(screen.queryByText('Guard scanner')).not.toBeInTheDocument()
  })

  it('routes GUARD users to the scanner and blocks administration', async () => {
    auth.user = { role: 'GUARD' }
    window.history.pushState({}, '', '/residents')
    render(<App />)
    expect(await screen.findByText('Guard scanner')).toBeInTheDocument()
    expect(screen.queryByText('Admin dashboard')).not.toBeInTheDocument()
  })

  it('allows ADMIN and blocks GUARD access to access history', async () => {
    auth.user = { role: 'ADMIN' }
    window.history.pushState({}, '', '/access-events')
    const adminView = render(<App />)
    expect(await screen.findByText('Access history')).toBeInTheDocument()
    adminView.unmount()

    auth.user = { role: 'GUARD' }
    render(<App />)
    expect(await screen.findByText('Guard scanner')).toBeInTheDocument()
    expect(screen.queryByText('Access history')).not.toBeInTheDocument()
  })

  it('allows only ADMIN users to open a resident detail route', async () => {
    auth.user = { role: 'ADMIN' }
    window.history.pushState({}, '', '/residents/resident-1')
    const adminView = render(<App />)
    expect(await screen.findByText('Resident detail')).toBeInTheDocument()
    adminView.unmount()

    auth.user = { role: 'GUARD' }
    render(<App />)
    expect(await screen.findByText('Guard scanner')).toBeInTheDocument()
    expect(screen.queryByText('Resident detail')).not.toBeInTheDocument()
  })

  it('does not expose routes for backend capabilities that are not implemented', async () => {
    auth.user = null
    window.history.pushState({}, '', '/forgot-password')
    render(<App />)
    expect(await screen.findByText('Bienvenido a SIGRA')).toBeInTheDocument()
    expect(screen.queryByText('Recuperar acceso')).not.toBeInTheDocument()
    expect(screen.queryByText(/olvidó su contraseña/i)).not.toBeInTheDocument()
  })

  it('denies signed-out users protected routes', async () => {
    auth.user = null
    window.history.pushState({}, '', '/dashboard')
    render(<App />)
    expect(await screen.findByText('Bienvenido a SIGRA')).toBeInTheDocument()
    expect(screen.queryByText('Admin dashboard')).not.toBeInTheDocument()
  })

  it('invalidates unsupported authenticated roles instead of rendering protected content', async () => {
    auth.user = { role: 'RESIDENT' }
    window.history.pushState({}, '', '/dashboard')
    render(<App />)
    expect(await screen.findByText('Bienvenido a SIGRA')).toBeInTheDocument()
    expect(auth.invalidateSession).toHaveBeenCalledWith('Unsupported session role.')
    expect(screen.queryByText('Admin dashboard')).not.toBeInTheDocument()
  })
})
